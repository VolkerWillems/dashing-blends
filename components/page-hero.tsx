  ...
  return (
    <section className="section-shell grid gap-10 py-10 sm:py-14 lg:min-h-[calc(100dvh-88px)] lg:grid-cols-[0.86fr_1.14fr] lg:items-center lg:gap-14 lg:py-16 xl:gap-20">
      <div className="page-hero-copy flex flex-col gap-6 lg:pr-4">
        ...
        <h1 className="display-copy max-w-4xl text-[clamp(3.35rem,7.2vw,8rem)] text-foreground">{title}</h1>
        <p className="max-w-2xl text-base leading-7 text-muted-foreground sm:text-lg sm:leading-8 xl:text-xl">
          {introduction}
        </p>
      </div>

      <figure className="page-hero-image page-hero-media min-h-[500px] sm:min-h-[650px] lg:min-h-[74dvh]">
        <Image
          src={image}
          alt={imageAlt}
          fill
